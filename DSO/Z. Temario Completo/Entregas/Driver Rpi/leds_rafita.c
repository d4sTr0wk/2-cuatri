...
#define DRIVER_DESC   "Driver para placa Rpi"
...
static void byte2leds(char ch){
    int val = (int)ch;
    int aux = (int)ch;
    aux = (aux >> 6);  // Aquí en 'aux' guardo los bits 6 y 7.
    if(aux == 0){  // bits 6 y 7 a cero.
		int i;
		for(i = 0; i < 6; i++){
			gpio_set_value(LED_GPIOS[i], (val >> i) & 1);
		}
    } else if(aux == 1){    // bit 6 a uno y bit 7 a cero.
		int i;
		for(i = 0; i < 6; i++){
			if(((val >> i) & 1) == 1){
				gpio_set_value(LED_GPIOS[i], (val >> i) & 1);
	...
	} else if(aux == 2){    // bit 6 a cero y bit 7 a uno.
		int i;
		for(i = 0; i < 6; i++){
			if(((val >> i) & 1) == 1){
	...
	} else {  // bits 6 y 7 a uno.
		printk(KERN_INFO "NO está definido. \n");
	}
}
